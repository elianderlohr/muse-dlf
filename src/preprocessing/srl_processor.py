...
class SRLProcessor:
    def __init__(self, df, dataframe_path="../notebooks/classifier/X_srl_filtered.pkl", force_recalculate=False, save_type='pickle'):
        """
        ...
        if self.save_type not in ['csv', 'pickle', 'json']:
            raise ValueError("Invalid save_type. Must be one of 'csv', 'pickle', or 'json'.")
    ...
    def _recalculate_srl(self):
        ...
        self.df['text'] = self.df['text'].apply(lambda x: x[:480])  # Ensuring sentences are not too long
    ...
    def _extract_srl_batch(self, batched_sentences, predictor):
        ...
        for srl in batched_srl:
            ...
            for verb_entry in srl['verbs']:
                ...
                }) if arg_components['ARG0'] or arg_components['ARG1'] else {'predicate': '', 'ARG0': '', 'ARG1': ''}
            results.append(sentence_results if sentence_results else [{'predicate': '', 'ARG0': '', 'ARG1': ''}])
        return results
