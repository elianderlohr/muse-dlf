...
class CoreUtil:
    ...
    def _moment_dict(data, k, data_mean=None):
        ...
        return sum(data[key] * (key - data_mean) ** k for key in data) / sum(
            data.values()
        )
            ...
            if averages:
                results.append(
                    [
                        axis[0],
                        axis[1],
                        CoreUtil._moment_dict(data, 4, averages[axis])
                        / (CoreUtil._moment_dict(data, 2, averages[axis]) ** 2)
                        - 3,
                    ]
                )
            else:
                results.append(
                    [
                        axis[0],
                        axis[1],
                        CoreUtil._moment_dict(data, 4)
                        / (CoreUtil._moment_dict(data, 2) ** 2)
                        - 3,
                    ]
                )
        return results
    ...
    def compute_statistical_significance(result, samples):
        ...
        with open(result) as fi:
            for line in fi:
                ...
                axis_rank_table.append(
                    [pole0, pole1, percent_rank, value, np.mean(sample_values)]
                )
