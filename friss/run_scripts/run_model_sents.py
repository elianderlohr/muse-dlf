...
from friss.model.sents_model import *
...
MODEL_CLASSES_TYPES = {"friss": True, "roberta_baseline": False}
...
def train(model, tokenizer, fold, dataset_splits, tb_writer):
            ...
            optimizer_grouped_parameters.append(
                {
                    "params": [
                        p
                        for n, p in model.dictionary.named_parameters()
                        if not any(nd in n for nd in no_decay)
                    ],
                    "lr": config_args["dictionary_lr"],
                    "weight_decay": config_args["weight_decay"],
                }
            )
            optimizer_grouped_parameters.append(
    ...
    def pre_training_setup():
        train_dataset, sampler = load_sliced_dataset(
            dataset_splits,
            fold,
            "train",
        ...
        t_total = (
            ...
            // config_args["gradient_accumulation_steps"]
            * config_args["num_train_epochs"]
        )
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer,
            num_warmup_steps=config_args["warmup_steps"],
    ...
    def log_before_training():
        ...
        logger.info(
            ...
            config_args["train_batch_size_labeled"]
            + config_args["train_batch_size_unlabeled"],
        )
        logger.info(
    ...
    def write_results_to_eval_file(
        v_measure, mcc, confusion_matrix, class_report, acc, f1, prefix, mode="test"
    ):
        with open(config_args["output_eval_file"], "a+") as writer:
            writer.write(
                "mode-{}-iter-{}-issue-{}-fold-{}-accuracy-{}-f1-{}\n".format(
                    mode, prefix, config_args["issue"], fold, acc, f1
                )
            )
            writer.write("mcc:\n{}\n".format(mcc))
    ...
    def log_metrics(model, tokenizer, fold, dataset, best_acc, global_step, mode):
        eval_losses_avg, mcc, confusion_matrix, class_report, acc, f1 = validate_test(
            ...
            dataset,
            mode,
            dictionary=MODEL_CLASSES_TYPES[config_args["model_type"]],
            analysis=False,
        )
        ...
        for param_group_index, optimizer_param_group in enumerate(
            optimizer.param_groups
        ):
            tb_writer.add_scalar(
                f"LearningRate/{param_group_index}",
                optimizer_param_group["lr"],
                global_step,
            )
        ...
        if acc > best_acc:
            ...
            torch.save(
                config_args,
                os.path.join(config_args["output_dir"], "training_args.bin"),
            )
    ...
    for _ in train_iterator:  # Iterating over pre set number of epochs
        for step, batch in enumerate(tqdm(train_dataloader)):
            batch = [
                item.to(device) if type(item) == torch.Tensor else item
                for item in batch
            ]
            ...
            if config_args["gradient_accumulation_steps"] > 1:
                ...
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), config_args["max_grad_norm"]
                )
            tr_loss += loss.item()
            if (step + 1) % config_args["gradient_accumulation_steps"] == 0:
                ...
                if (
                    config_args["logging_steps"] > 0
                    and global_step % config_args["logging_steps"] == 0
                ):
                    # Get the train accuracy and write it to eval file
                    tb_writer.add_scalar(f"Loss/train", loss, global_step)
                    if MODEL_CLASSES_TYPES[config_args["model_type"]]:
                        tb_writer.add_scalar(
                            "CELossSent/train", ce_loss_sent, global_step
                        )
                        tb_writer.add_scalar(
                            "CELossDic/train", ce_loss_dic, global_step
                        )
                        tb_writer.add_scalar(
                            "TripletLoss/train", triplet_loss, global_step
                        )
                    ...
                    (
                        ...
                        tr_preds, tr_out_label_ids, config_args["num_labels"]
                    )
                    tr_v_measure = -1
                    write_results_to_eval_file(
                        tr_v_measure,
                        tr_mcc,
                        tr_confusion_matrix,
                        tr_class_report,
                    ...
                    if previous_best_acc < acc:
                        ...
                        if early_stop == config_args["early_stop"]:
                            return (
                                eval_loss,
                                previous_best_acc,
                                macro_f1_associated_with_previous_best_acc,
                            )
                    model.train()

    last_loss, last_acc, last_f1 = log_metrics(
        model, tokenizer, fold, dataset_splits, previous_best_acc, global_step, "test"
    )
...
def validate_test(
    ...
    for i, batch in enumerate(tqdm(eval_dataloader)):
        # torch.cuda.empty_cache()
        batch = [
            item.to(device) if type(item) == torch.Tensor else item for item in batch
        ]
        ...
        with torch.no_grad():
            ...
            if dictionary:
                (
                    outputs,
                    dictionary_gumbel_weights_list,
                    top_k_tags_list,
                    top_k_metrics_and_indices_tags,
                    top_k_tags_list_metrics,
                ) = model.evaluate(tokenizer, batch)
            else:
        ...
        nb_eval_steps += 1
        preds = (
            np.append(preds, log_probs.cpu().numpy(), axis=0)
            if preds is not None
            else log_probs.cpu().numpy()
        )
        out_label_ids = (
            np.append(out_label_ids, batch[6].cpu().numpy(), axis=0)
            if out_label_ids is not None
            else batch[6].cpu().numpy()
        )
...
def build_args_parser():
    main_arg_parser = argparse.ArgumentParser(description="Parser for FRISS")
    main_arg_parser.add_argument(
        "--num_workers", type=str, default="0", help="num of workers for dataloader"
    )
    main_arg_parser.add_argument(
        "--issue",
        type=str,
    ...
    main_arg_parser.add_argument(
        "--fold", type=str, required=True, help="specify the fold index (0-9)"
    )
    main_arg_parser.add_argument(
        "--run_name",
        type=str,
        required=True,
        help="specify an unique identifier for the run",
    )
    main_arg_parser.add_argument(
        "--num_runs",
        type=int,
        default=3,
        help="specify how many times to run for each fold",
    )
    main_arg_parser.add_argument(
        "--mlm_identifier",
        type=str,
        default="11-19-2020-00-54-13",
    ...
    main_arg_parser.add_argument(
        "--train_batch_size_labeled",
        type=int,
        default=8,
        help="train_batch_size_labeled",
    )
    main_arg_parser.add_argument(
        "--train_batch_size_unlabeled",
        type=int,
        default=0,
        help="train_batch_size_unlabeled",
    )
    ...
    main_arg_parser.add_argument(
        ...
        type=str,
        required=True,
        help="spcify the num_unlabelld (0-41966)",
    )
    ...
    main_arg_parser.add_argument(
        "--classifier_lr", type=float, default=5e-4, help="the classifier learning rate"
    )
    main_arg_parser.add_argument(
        "--dictionary_lr", type=float, default=5e-4, help="the dictionary learning rate"
    )
    main_arg_parser.add_argument(
        "--alpha", type=float, default=1.0, help="manual tuning param alpha"
    )
    main_arg_parser.add_argument(
        "--beta", type=float, default=1.0, help="manual tuning param beta"
    )
    main_arg_parser.add_argument(
    ...
    main_arg_parser.add_argument(
        "--dictionary_learning_dropout_prob",
        type=float,
        default=0.2,
        help="the dictionary learning dropout rate",
    )
    main_arg_parser.add_argument(
        "--weight_decay", type=float, default=5e-7, help="optimizer weight decay"
    )

    main_arg_parser.add_argument(
        "--margin", type=float, default=3.0, help="the margin for triplet loss"
    )
    main_arg_parser.add_argument(
        "--num_k", type=int, default=1, help="gumbel softmax top-k parameter"
    )
    main_arg_parser.add_argument(
        "--schedule_r",
        type=float,
    ...
    config_args["output_dir"] = os.path.join(
...
def load_pretrained_model(start_from="raw"):
    ...
    if start_from == "mlm":
        ...
        config = config_class.from_pretrained(
            ...
            num_labels=config_args["num_labels"],
            finetuning_task=config_args["task_name"],
        )
        checkpoint = os.path.join(
            MFC_mlm_result_directory, config_args["mlm_identifier"]
        )
...
def main():
    ...
    # Starts training
    config_args["output_eval_file"] = os.path.join(
        config_args["output_dir"], "results.txt"
    )
    ...
    serialize_to_file(
        os.path.join(config_args["output_dir"], "run_config.json"), config_args
    )

    for run_index in range(config_args["num_runs"]):
        tb_writer = SummaryWriter(
            os.path.join(config_args["output_dir"], "runs/{}".format(run_index))
        )
        model, tokenizer = load_pretrained_model(start_from=config_args["start_from"])
        test_loss, acc, f1 = train(
            model, tokenizer, config_args["fold"], dataset_splits, tb_writer
        )
        accuracies.append(acc)
        f1s.append(f1)
        with open(final_result_file, "w") as f:
            f.write(
                "accuracies and f1s after {} runs for fold {}:{}:{} ".format(
                    run_index + 1, config_args["fold"], accuracies, f1s
                )
            )
    logger.info(
        "max accuracy and f1 over {} runs for fold {}:{}:{} ".format(
            config_args["num_runs"], config_args["fold"], max(accuracies), max(f1s)
        )
    )
