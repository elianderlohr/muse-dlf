from friss.model.model_utils import *
...
class RoBerta_baseline(BertPreTrainedModel):
    ...
    def forward(self, batch, isTrain=True):
        ...
        sent_mean_tokens_embeddings_by_sent = (
            torch.sum(roberta_tokens_embeddings_by_sent, dim=1) / num_tokens_by_sent
        )
        log_probs_sent = self.classifier(
            sent_mean_tokens_embeddings_by_sent,
        ...
        ce_loss_sent = self.frame_classification_loss_sent(
            log_probs_sent.contiguous().view(-1, self.num_labels), batch_labels
        )
...
class FRISS(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dictionary = DictionaryLearningGumbelSoftmaxMultiView(
            config, v=len(self.tags), n=2, k=self.num_k
        )
        self.classifier = GlobalAttentionHeadReluMean(config)
    ...
    def forward(self, batch, isTrain=True):
        ...
        if global_step % 50 == 0:
            self.gumbel_temperature = max(
                self.temperature_limit, math.exp(self.schedule_r * global_step)
            )

        num_tokens_by_sent = batch[1].sum(dim=1).float().contiguous().view(-1, 1)
        (
            sent_mean_tokens_embeddings_all_tags,
            tags_embeddings_all_tags,
            tags_numbers_by_sent,
            tags_numbers,
            tags_spans_by_tag,
        ) = get_selected_srl_units_embeddings(
            roberta_tokens_embeddings_by_sent,
            batch_roberta_doc_arg_verb_spans,
            num_tokens_by_sent,
            tags=self.tags,
        )
        ...
        dictionary_learning_input_all_tags = torch.cat(
            [tags_embeddings_all_tags, sent_mean_tokens_embeddings_all_tags], dim=1
        )
        (
            dictionary_weights,
            recontructed_embeddings,
            dictionary_logits,
            negative_frame_embeddings,
            reverse_dictionary_probs,
        ) = self.dictionary(
        ...
        dictionary_logits_by_tag_list = matrix_to_list_by_length(
            dictionary_logits, tags_numbers
        )

        (
            dictionary_logits_by_doc_list,
            pseudo_tags_padding_mask,
            tags_mask_by_doc,
        ) = batch_embeddings_by_doc_all_tags(
        ...
        dictionary_log_probs_by_doc_list = [
            dictionary_logits_by_doc_list[i] * tags_mask_by_doc[tag]
            for i, tag in enumerate(available_tags)
        ]
        dictionary_log_probs_by_doc = torch.stack(
            dictionary_log_probs_by_doc_list, dim=0
        )  # num_dictionaries x num_docs x num_frames
        ...
        tags_embeddings_all_tags_list = matrix_to_list_by_length(
            tags_embeddings_all_tags, tags_numbers
        )
        recontructed_embeddings_all_tags_list = matrix_to_list_by_length(
            recontructed_embeddings, tags_numbers
        )
        for i, tags_embeddings_one_tag in enumerate(tags_embeddings_all_tags_list):
            tag_size = tags_embeddings_one_tag.size(0)
            positive_tags_embeddings_all_tags.append(
                tags_embeddings_one_tag.repeat(self.num_labels - self.num_k, 1)
            )
            reconstructed_embeddings_all_tags.append(
                recontructed_embeddings_all_tags_list[i].repeat(
                    self.num_labels - self.num_k, 1
                )
            )
            neg_tags_embeddings_one_tag = list()
            for j in range(self.num_labels - self.num_k):
                neg_tags_embeddings_one_tag.append(
                    tags_embeddings_one_tag[torch.randperm(tag_size)]
                )
        ...
        positive_tags_embeddings_all_tags = torch.cat(
            positive_tags_embeddings_all_tags, dim=0
        )
        negative_tags_embeddings_all_tags = torch.cat(
            negative_tags_embeddings_all_tags, dim=0
        )
        reconstructed_embeddings_all_tags = torch.cat(
            reconstructed_embeddings_all_tags, dim=0
        )
        triplet_loss = self.triplet_loss(
            reconstructed_embeddings_all_tags,
            positive_tags_embeddings_all_tags,
    ...
    def evaluate(self, tokenizer, batch):
        ...
        tags_spans_by_tag = [
            tag_spans
            for i, tag_spans in enumerate(tags_spans_by_tag)
            if tags_numbers[i] > 0
        ]
        tags_numbers_by_sent = [
            tag_num
            for i, tag_num in enumerate(tags_numbers_by_sent)
            if tags_numbers[i] > 0
        ]

        (
            batch_srl_tags_ids,
            batch_srl_tags_texts,
            batch_srl_sents_tags_texts,
        ) = get_texts_by_tags(
        ...
        gumbel_temperature = (
            self.evaluate_gumbel_temperature
            if self.evaluate_gumbel_temperature > 0
            else self.gumbel_temperature
        )
        ...
        (
            ...
            negative_frame_embeddings,
            reverse_dictionary_probs,
            top_k_weights_and_indices_tags,
            top_k_tags_list,
            top_k_metrics_and_indices_tags,
            top_k_tags_list_metric,
        ) = self.dictionary.evaluate(
            dictionary_learning_input_all_tags,
            tags_numbers,
            batch_srl_tags_texts,
            batch_srl_sents_tags_texts,
