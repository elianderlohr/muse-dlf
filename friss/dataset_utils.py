...
class MySampler(Sampler):
    def __init__(
        self, fold_indices, batch_size_labeled=8, batch_size_unlabeled=8, shuffle=True
    ):
        ...
        self.unlabeled_indices = [
            i for i in self.fold_indices if i >= self.num_labelled
        ]
        self.relative_labeled_indices = list(range(0, len(self.labeled_indices)))
        self.relative_unlabeled_indices = list(
            range(len(self.labeled_indices), len(self.fold_indices))
        )
        ...
        self.num_batches_labelled = math.ceil(
            len(self.labeled_indices) / batch_size_labeled
        )
        if batch_size_unlabeled > 0 and len(self.unlabeled_indices) > 0:
            self.num_batches_unlabelled = math.ceil(
                len(self.unlabeled_indices) / batch_size_unlabeled
            )
            self.num_batches = max(
                self.num_batches_labelled, self.num_batches_unlabelled
            )
    ...
    def __iter__(self):
        ...
        while n > 0:
            ...
            batch_labeled_indices = self.relative_labeled_indices[
                i
                * self.batch_size_labeled : min(
                    (i + 1) * self.batch_size_labeled, self.num_labelled_subset
                )
            ]
            ...
            batch_unlabeled_indices = self.relative_unlabeled_indices[
                j
                * self.batch_size_unlabeled : min(
                    (j + 1) * self.batch_size_unlabeled, self.num_unlabelled_subset
                )
            ]
...
def load_sliced_dataset(
    dataset_splits,
    fold,
    mode,
    batch_size_labeled=None,
    ...
    sliced_mfc_sents_dataset = Subset(
        mfc_sents_dataset, torch.LongTensor(labeled_indices)
    )
