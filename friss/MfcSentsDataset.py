...
class MfcSentsDataset(Dataset):
    def __init__(self, root, name):
        ...
        self.input_file_names = [
            os.path.join(self.root, "raw", input_file_name)
            for input_file_name in self.get_input_file_names()
        ]
        self.raw_data = list()
        ...
        for i in range(len(self.raw_data)):
            if i < self.num_labelled:
                ...
                self.doc_labels.append(
                    0
                )  # for unlabelled data, assume the label is 0 and use mask during loss

        self.roberta_doc_arg_verb_spans_for_all_docs_path = os.path.join(
            self.root, "processed", "roberta_doc_arg_verb_spans_for_all_docs.json"
        )
        self.dataset_size = len(self.raw_data)
        self.processed_roberta_path = os.path.join(
            MFC_graph_dataset_processed_directory, "mfc_roberta_sents_features.pt"
        )
        if os.path.exists(self.processed_roberta_path):
            ...
            self.roberta_doc_arg_verb_spans_for_all_docs = (
                self.convert_word_span_to_roberta_token_span()
            )

    def get_input_file_names(self):
        return [
            "mfc_graphs_sents_complete_immigration_5933.json",
    ...
    def preprocess_roberta_features(self):
        ...
        for i in tqdm(range(self.dataset_size)):
            ...
            for sent_words in doc_words:
                ...
                for j, t in enumerate(sent_words):
                    if j != 0:
                        temp_ = tokenizer.tokenize(" ".join(["X", t]))[
                            1:
                        ]  # Remove the X token
                    else:
                    ...
                    start_offsets_one_sent.append(
                        cumulative + 1
                    )  # Pos 0 is reserved for <s>
        ...
        (
            ...
            all_segment_ids,
            all_label_ids,
        ) = load_bert_features_sents(
            config_args["task_name"],
            tokenizer,
            self.doc_labels,
            text_a_tokens_for_all_docs=text_a_tokens_for_all_docs,
        )
    ...
    def span_convert(
        self, predicate_arg_verbs_dict, sent_start_offsets, sent_end_offsets
    ):
        roberta_predicate_arg_verbs_dict = dict()
        for k, v in predicate_arg_verbs_dict.items():
            roberta_predicate_arg_verbs_dict[k] = [
                min(
                    sent_start_offsets[v[0][0]],
                    (int(config_args["max_seq_length"]) - 1),
                ),
                min(
                    sent_end_offsets[v[0][1]], (int(config_args["max_seq_length"]) - 1)
                ),
            ]
    ...
    def convert_word_span_to_roberta_token_span(self):
        ...
        for i in tqdm(range(len(self.roberta_features["start_offsets_for_all_docs"]))):
            ...
            for j, sent_arg_verb_spans in enumerate(doc_arg_verb_spans):
                ...
                for k, predicate_arg_verbs_dict in enumerate(sent_arg_verb_spans):
                    # print("k " + str(k))
                    roberta_sent_arg_verb_spans.append(
                        self.span_convert(
                            predicate_arg_verbs_dict,
                            self.roberta_features["start_offsets_for_all_docs"][i][j],
                            self.roberta_features["end_offsets_for_all_docs"][i][j],
                        )
                    )
        ...
        serialize_to_file(
            self.roberta_doc_arg_verb_spans_for_all_docs_path,
            roberta_doc_arg_verb_spans_for_all_docs,
        )
        return roberta_doc_arg_verb_spans_for_all_docs

    def __getitem__(
        self, index
    ):  # This is absolute index in terms of the entire dataset
        doc_id = index
        doc_type = (
            1 if index < self.num_labelled else 0
        )  # 0 means unlabelled and 1 means labelled
        doc_label_id = self.doc_labels[index]
        cumulative_doc_start_index = self.roberta_features[
            "cumulative_doc_start_indices"
        ][index]
        cumulative_doc_end_index = self.roberta_features["cumulative_doc_end_indices"][
            index
        ]
        ...
        roberta_input_ids = self.roberta_features["all_input_ids"][
...
def collate(batch):
    ...
    batch_cumulative_sents_start_indices_by_doc = torch.LongTensor(
        batch_cumulative_sents_start_indices_by_doc
    )
    batch_cumulative_sents_end_indices_by_doc = torch.LongTensor(
