...
class MulticlassProcessor(object):
    ...
    def create_examples_sents(self, text_a_tokens_for_all_docs):
        ...
        for i, text_a_tokens_for_one_doc in enumerate(text_a_tokens_for_all_docs):
            ...
            for (
                text_a_tokens
            ) in text_a_tokens_for_one_doc:  # text_a_tokens is for one sentence
                examples.append(
                    InputExample(
                        guid=guid,
                        text_a=None,
...
def convert_example_to_feature_roberta_base(
    example_row,
    pad_token=0,
...
def convert_examples_to_features(
    ...
    with Pool(process_count) as p:
        features = list(
            tqdm(
                p.imap(
                    convert_example_to_feature_roberta_base, examples, chunksize=500
                ),
                total=len(examples),
            )
        )
...
def compute_metrics(preds, labels, num_labels, false_pos=None, false_neg=None):
    top_k_best_preds_values, top_k_best_preds_indices = torch.topk(
        torch.Tensor(preds), 3, dim=1
    )
    best_preds = top_k_best_preds_indices[:, 0].numpy()

    assert len(best_preds) == len(labels)
    mcc = matthews_corrcoef(labels, best_preds)
    conf_mx = multilabel_confusion_matrix(
        labels, best_preds, labels=list(range(num_labels))
    )
...
class TopKMeter(object):
    ...
    def update_above_threshold(self, weights, keys, values):
        for i in range(len(weights)):
            if weights[i] > self.g:
                ...
                self.items_above_the_threshold[processed_key].append(
                    (values[i], weights[i])
                )
    ...
    def get_items_above_threshold(self, k=500):
        ...
        i = 0
        for key in sorted(
            self.items_above_the_threshold,
            key=lambda x: len(self.items_above_the_threshold[x]),
            reverse=True,
        ):
            if i == k:
                break
            l = self.items_above_the_threshold[
                key
            ]  # a list of tuples (value, weight), ideally sort it by weight
    ...
    def update(self, weights, keys, values, reverse=True):
        ...
        temp_w = list()
        for w, k, v in sorted(
            zip(self.top_k_weights, self.top_k_keys, self.top_k_values), reverse=reverse
        ):
            temp_w.append(w)
            temp_k.append(k)
            temp_v.append(v)
    ...
    def update_no_duplicates(self, weights, keys, values):
        ...
        for w, k, v in sorted(
            zip(
                self.top_k_weights_no_duplicates,
                self.top_k_keys_no_duplicates,
                self.top_k_values_no_duplicates,
            ),
            reverse=True,
        ):
            temp_w.append(w)
...
def load_original_data():
    ...
    dataset = _read_tsv(
        os.path.join(config_args["data_dir"], str(config_args["issue"]) + ".tsv")
    )
    return dataset
...
def load_data_with_validation(n_splits):
    ...
    if os.path.exists(
        os.path.join(
            config_args["data_dir"], "fold_train_valid_test_split_indices.json"
        )
    ):
    ...
    time = 0
    for train_indices, test_indices in kf.split(
        lines, y
    ):  # ([0,1,...,1000],[1001,...,9999]) () () ...
        dataset[time] = dict()
        ...
        X_train, X_valid = train_test_split(
            dataset[time]["train"], test_size=0.1, random_state=68, stratify=train_y
        )
...
def load_data(n_splits):
    # load dataset and create a split
    if os.path.exists(
        os.path.join(config_args["data_dir"], "fold_train_test_split_indices.json")
    ):
        return deserialize_from_file(
            os.path.join(config_args["data_dir"], "fold_train_test_split_indices.json")
        )
    ...
    for train_indices, test_indices in kf.split(
        ...
        assert train_indices.shape[0] == len(
            dataset_indices[fold_index]["train_indices"]
        )
        fold_index += 1
    serialize_to_file(
        os.path.join(config_args["data_dir"], "fold_train_test_split_indices.json"),
...
def load_bert_features_sents(
    task, tokenizer, doc_labels, text_a_tokens_for_all_docs=None
):
...
RAW_SRL_FILE_PATH = os.path.join(
    MFC_dataset_directory, "{}/{}_bert_srl.json".format(issue, issue)
)
RAW_COREF_FILE_PATH = os.path.join(
    MFC_dataset_directory, "{}/{}_bert_coref.json".format(issue, issue)
)
RAW_COREF_SENTS_FILE_PATH = os.path.join(
    MFC_dataset_directory, "{}/{}_bert_coref_sents.json".format(issue, issue)
)
RAW_SRL_SENTS_FILE_PATH = os.path.join(
    MFC_dataset_directory, "{}/{}_bert_srl_sents.json".format(issue, issue)
)
RAW_SRL_SENTS_UNLABELLED_FILE_PATH = os.path.join(
    MFC_dataset_directory,
    "{}/{}_bert_srl_sents.json".format(
        "immigration_unlabelled", "immigration_unlabelled"
    ),
)
RAW_SRL_SENTS_UNSHUFFLED_FILE_PATH = os.path.join(
    MFC_dataset_directory,
    "{}/{}_bert_srl_sents.json".format(
        "immigration_unshuffled", "immigration_unshuffled"
    ),
)
...
processors = {"MFC_multiclass_classification": MulticlassProcessor}
